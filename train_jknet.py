...
import gc
from models.jknet import JKNetConcat, JKNetMaxPooling, JKNetLSTMAttention

device = 'cuda' if torch.cuda.is_available() else 'cpu'
device = 'cpu'

experiment_name = 'JKNet'
...
def train_config(
    ...
    if model == "SimpleGNN":
        ...
        # GAT
        model = JKNetMaxPooling(embedding_dim=embedding_dim, intermediate_dim=intermediate_dim,
                                encoding_dim= intermediate_dim, n_layers=layers, 
                                layer_module=gnn.GATConv, activation=activation_function, heads=heads)
    elif model == 'JKNetConcat':
        # GAT
        model = JKNetConcat(embedding_dim=embedding_dim, intermediate_dim=intermediate_dim,
                                encoding_dim= intermediate_dim, n_layers=layers, 
                                layer_module=gnn.GATConv, activation=activation_function, heads=heads)
    elif model == 'JKNetLSTMAttention':
        # GAT
        model = JKNetLSTMAttention(embedding_dim=embedding_dim, intermediate_dim=intermediate_dim,
                                encoding_dim= intermediate_dim, n_layers=layers, 
                                layer_module=gnn.GATConv, activation=activation_function, heads=heads, bidirectional=True)
    model.to(device)
    # optimizer
...
models =['JKNetConcat', 'JKNetMaxPooling', 'JKNetLSTMAttention']
