...
BASE_PATH = 'datasets/tadw-sc/krogan-extended'
df = pd.read_csv(os.path.join(BASE_PATH, 'krogan-extended.csv'),index_col=0)
#%%
...
#%%
f = open(os.path.join(BASE_PATH,'krogan_extended_proteins_list.txt'),'w')
...
with open('datasets/colins_proteins_list.txt','w') as f:
    f.write(string_proteins)
#%%
sgd_results = pd.read_csv(os.path.join(BASE_PATH,'sgd_results.csv'))
#%%
...
with open(os.path.join(BASE_PATH,'semantic_name_to_sgd_id.json'),'w') as f:
    json.dump(semantic_name_to_sgd_id, f)


with open(os.path.join(BASE_PATH,'sgd_id_to_semantic_name.json'),'w') as f:
